/**
 * Created by Carter on 13-12-11.
 */
/**
 * map utility, a wrapper for jvectormap
 */
define(['jquery',
	'jvectormapworlden',
	'bowerjvectormap'
		...
		drawMap: function($el, pData/*{{"country_code":["cn","us"], "text":["China","USA"], "value":["10","20"]}}*/, onRegionClickCB) {
			...
			$el.vectorMap({
				...
				onMarkerLabelShow: function(event, label, index){
					label.html(
						data.text[index]+" "+data.value[index]
					);
				},
				...
				onRegionSelected: function(e, code, isSelected, selectedRegions){
					if(isSelected) {
						...
						onRegionClickCB && onRegionClickCB(code.toLowerCase());
					}
				}
			});

			var mapObject = $el.vectorMap('get', 'mapObject');
		}
	};



	function convertCountryCodeToLatLon(codeArray) {
	   if(codeArray==null || !codeArray.length) {
		   throw new Error('codeArray cannot be null')
	   }
	   var pos = [];
	   for(var i=0; i<codeArray.length; i++) {
		        var p = countryLocation[codeArray[i]];
		        if(p==null) {
			        throw new Error('unknown country code: ' + codeArray[i] );
		        }
		        pos.push([p['lat'], p['long']])

	   }
		return pos;
	};

	function matrixTransform(data){
	   if(data==null || !data.length) {
		   throw new Error('data cannot be null');
	   }
	   var transformed = {"country_code":[], "text":[], "value":[]};
	   for(var i= 0, len=data.length; i<len; i++) {
		   transformed['country_code'].push(data[i]['country_code']);
		   transformed['text'].push(data[i]['text']?data[i]['text']:'');
		   transformed['value'].push(data[i]['value']);
	   }
	   return transformed;
	}
